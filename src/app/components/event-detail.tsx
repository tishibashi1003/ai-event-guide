...
import { ArrowLeft, MapPin, Calendar, Users, Tag, ChevronDown, MapIcon } from "lucide-react"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main className="flex-1 px-4 py-6">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {event.categories.map((category, index) => (
            <span key={index} className="px-3 py-1 bg-[#FFD700] text-black rounded-full">
              {category}
            </span>
          ))}
        </div>
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-black mb-2">イベント詳細</h2>
      ...
      <footer className="p-4">
        <button className="w-full bg-[#FFD700] text-black py-3 rounded-lg font-semibold">ココいく！</button>
      </footer>
    </div>
  )
}
